...
class BatteryClawEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, A=1.0, B=2.0, C=0.5, D=0.4,
                 episode_steps=180, randomize=True, difficulty=3):
        super().__init__()
        self.A, self.B, self.C, self.D = A, B, C, D
        self.episode_steps = episode_steps
        self.render_mode   = render_mode
        self.randomize     = randomize
        # [F4] CURRICULUM: do kho tang dan khi train.
        #  1 = de  : 1 may co dinh (ultrabook), it workload, KHONG spike/delay/sac ngau nhien
        #  2 = vua : 6 may + 12 workload, CHUA bat spike/action-delay
        #  3 = full: bat het (spike, action delay, sac rut ngau nhien, pin phi tuyen)
