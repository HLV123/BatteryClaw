...
def _machine_id_stable():
    """ID bền: uu tien serial o dia + UUID mainboard (on dinh hon MAC address).
    MAC (uuid.getnode) doi khi may nhieu card mang/VPN -> khoa nham key."""
    parts = []
    try:
        import subprocess
        # UUID mainboard (on dinh nhat)
        out = subprocess.run(
            ["wmic", "csproduct", "get", "UUID"],
            capture_output=True, text=True, timeout=5)
        for line in out.stdout.splitlines():
            line = line.strip()
            if line and line.upper() != "UUID" and "FFFFFFFF" not in line.upper():
                parts.append(line)
                break
    except Exception:
        pass
    try:
        ...
        pass
    # processor + hostname (luon co, lam phan bo sung)
...
def machine_id():
    """ID may chinh (ben). Neu khong lay duoc phan cung -> fallback ID cu."""
    parts = _machine_id_stable()
    # neu chi co processor+node (2 phan) -> khong doc duoc phan cung -> dung ID cu
    if len([p for p in parts if p]) <= 2:
        return _machine_id_legacy()
    raw = "|".join(p for p in parts if p)
    return hashlib.sha256(raw.encode()).hexdigest()[:16]


def _machine_id_legacy():
    """ID kieu cu (uuid.getnode) — giu de tuong thich nguoc key da activate."""
    import uuid
...
class LicenseGate:
    ...
    def _check_existing(self):
        ...
        Chap nhan ca machine_id moi (ben) lan cu (legacy) de khach da activate
        bang ID cu khong bi khoa khi cap nhat app."""
        saved = self.cfg.get("machine_id")
        id_match = saved in (self.mid, _machine_id_legacy())
        if self.cfg.get("key") and self.cfg.get("server_url") and id_match:
    ...
    def _verify_online(self):
        try:
            # dung machine_id da luu (cai server da khoa) de khop, khong phai mid moi
            mid_use = self.cfg.get("machine_id") or self.mid
            r = api_post(self.cfg["server_url"], "/api/verify",
                         {"key": self.cfg["key"], "machine_id": mid_use})
            if r.get("ok"):
                if r.get("expires_at"):
...
class BatteryClawApp:
    ...
    def _build_ui(self):
        ...
        self._maybe_show_charge_tip()

    def _model_for_profile(self):
        """[1.1] Chon file onnx theo profile khach chon tren UI.
        Neu chua train model rieng cho profile do -> fallback model mac dinh."""
        label = self.profile.get() if hasattr(self, "profile") else "Cân bằng"
        name = {
            "Tiết kiệm pin":  "batteryclaw_policy_battery_saver.onnx",
            "Hiệu năng cao":  "batteryclaw_policy_performance.onnx",
            "Cân bằng":       "batteryclaw_policy.onnx",
        }.get(label, "batteryclaw_policy.onnx")
        # tim file profile o cac vi tri quen thuoc
        for d in (os.path.join(BASE, "models"),
                  os.path.join(BASE, "simulator", "models")):
            p = os.path.join(d, name)
            if os.path.exists(p):
                return p
        return MODEL_PATH   # fallback model mac dinh (balanced) neu chua co file profile

    def _maybe_show_charge_tip(self):
        try:
            if self.cfg.get("charge_tip_shown"):
                return
            import subprocess
            out = subprocess.run(["wmic", "computersystem", "get", "Manufacturer"],
                                  capture_output=True, text=True, timeout=5)
            vendor = out.stdout.lower()
            tip = None
            if "micro-star" in vendor or "msi" in vendor:
                tip = ("Mẹo bảo vệ pin: máy MSI cho giới hạn sạc 80% trong MSI Center "
                       "(Battery Master / Battery Calibration). BatteryClaw không tự đặt "
                       "được vì Windows không cho phần mềm ngoài điều khiển sạc.")
            elif "lenovo" in vendor:
                tip = ("Mẹo: máy Lenovo có 'Conservation Mode' trong Lenovo Vantage để "
                       "giới hạn sạc bảo vệ pin.")
            elif "asus" in vendor:
                tip = ("Mẹo: máy ASUS có 'Battery Health Charging' trong MyASUS/Armoury "
                       "Crate để giới hạn sạc.")
            if tip:
                self.note.config(text=tip, fg=MUTED)
            self.cfg["charge_tip_shown"] = True
            save_cfg(self.cfg)
        except Exception:
            pass
    ...
    def start(self):
        ...
        try:
            ...
            model_path = self._model_for_profile()   # [1.1] chon onnx theo profile UI
