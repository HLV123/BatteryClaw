...
class OnnxPolicy:
    def __init__(self, model_path):
        ...
        log.info(f"  Input : {in_shape}")
        log.info(f"  Output: {self.sess.get_outputs()[0].shape}")

        # [2.1] Phat hien model LSTM dang chuoi: input 3 chieu (batch, seq, 15).
        #  Neu vay, giu rolling window seq state va feed ca chuoi moi buoc.
        self.is_seq = len(in_shape) == 3
        if self.is_seq:
            self.seq_len = int(in_shape[1]) if isinstance(in_shape[1], int) else 30
            self._seq_buf = np.zeros((self.seq_len, OBS_DIM), dtype=np.float32)
            log.info(f"  -> LSTM mode: rolling window {self.seq_len} state")
        else:
            self.seq_len = 0

    def predict(self, obs: np.ndarray) -> np.ndarray:
        """obs (15,) -> raw tanh action (7,). Ho tro ca model phang lan LSTM chuoi."""
        if self.is_seq:
            # day state moi vao rolling window, feed ca chuoi
            self._seq_buf = np.roll(self._seq_buf, shift=-1, axis=0)
            self._seq_buf[-1] = obs.astype(np.float32)
            inp = self._seq_buf.reshape(1, self.seq_len, OBS_DIM)
...
def state_to_obs(s: dict) -> np.ndarray:
    ...
    # [3.3 da may] mot so may (man ngoai/driver) khong doc duoc brightness -> engine
    #  tra -1. Dung 80 trung tinh thay vi de model hieu sai man hinh tat.
    ...
    if refresh_hz is None or refresh_hz < 60:   # [3.3] khong doc duoc -> 60Hz trung tinh
        refresh_hz = 60
    ...
    return np.array([
        ...
        float(np.clip((refresh_hz - 60) / (144 - 60), 0.0, 1.0)),  # PANEL_MAX_HZ=144, khop battery_env
        wifi,
        audio,
...
def action_to_command(raw_action: np.ndarray, state: dict) -> dict:
    ...
    # Dang sac dien -> noi long tiet kiem NHE (de AI tu quyet phan lon, chi dat san
    #  an toan thap de khong ep cung che mat quyet dinh cua AI).
    if state.get("plugged", False):
        throttle_max = max(throttle_max, 0.60)
        brightness   = max(brightness,   0.45)
        defer        = False
        gpu_switch   = max(gpu_switch, 1)   # khong ep tat dGPU khi cam dien
    ...
    # map refresh mode (0/1/2) -> Hz that cho engine dat ChangeDisplaySettings
    _refresh_hz = {0: 60, 1: 120, 2: 144}.get(refresh_mode, 60)
    return {
        "type"        : "action",
        ...
        "fg_app_path" : state.get("fg_app_path", ""),  # [3.2] app de dat GPU preference
        "refresh_rate": refresh_mode,    # 0=60,1=120,2=max (giu cho tuong thich)
        "refresh_hz"  : _refresh_hz,     # Hz that cho engine dat
        "wifi_save"   : wifi_save,
