...
public static class HardwareControl
{
    ...
    // ── GPU PREFERENCE per-app (registry, hop le) ───────────────────────────
    //  Windows khong cho ep tat dGPU toan cuc, nhung cho dat preference TUNG app
    //  (giong Settings > Graphics). GpuPreference=1: power saving (iGPU);
    //  =2: high performance (dGPU). Da xac nhan ghi duoc tren may that.
    private const string GpuPrefKey =
        @"Software\Microsoft\DirectX\UserGpuPreferences";
    private static string _lastGpuApp = null;
    private static int _lastGpuPref = -1;

    public static bool SetAppGpuPreference(string appPath, bool powerSaving)
    {
        if (string.IsNullOrEmpty(appPath)) return false;
        int pref = powerSaving ? 1 : 2;
        if (appPath == _lastGpuApp && pref == _lastGpuPref) return true;
        try
        {
            using var k = Microsoft.Win32.Registry.CurrentUser.CreateSubKey(GpuPrefKey);
            k.SetValue(appPath, $"GpuPreference={pref};");
            _lastGpuApp = appPath; _lastGpuPref = pref;
            return true;
        }
        catch { }
        return false;
