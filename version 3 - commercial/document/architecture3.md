...
```
  ...
  ────────────────────────────────────────        ──────────────────────────
   ...
   3  workload_group      11  wifi_active           3  gpu_switch    per-app ✓
   4  brightness          12  audio_active          4  refresh_mode  {0,1,2}
   5  throttle_max        13  ram_pressure          5  wifi_save     {0,1}
   6  time_norm           14  time_of_day           6  charge_limit  (hướng dẫn*)
   7  gpu_type

   ★ discharge: tín hiệu quan trọng nhất; lấy từ WMI DischargeRate thật.
   gpu_switch: đặt GPU preference PER-APP cho app foreground (iGPU tiết kiệm /
     dGPU hiệu năng) qua registry — hợp lệ, KHÔNG ép tắt dGPU toàn cục.
   * charge_limit: không có API Windows chuẩn; máy MSI/Lenovo/ASUS chỉ đặt được
     qua app hãng. App phát hiện hãng → HƯỚNG DẪN người dùng bật trong MSI Center
     (thay vì giả vờ làm).
```

refresh_mode (0/1/2) → brain map sang Hz thật (60/120/144) gửi engine.
gpu_switch (0=iGPU/1=dGPU) → brain gửi kèm fg_app_path → engine đặt preference.
...
├── simulator/       battery_env.py (12 workload, 6 máy, F1-F8) + train.py (curriculum,
│                    --profile cho 3 model) + train_sac.py + export_lstm_onnx.py
├── brain/           rl_brain.py — state_to_obs, ONNX infer (phẳng + LSTM chuỗi), online
├── engine_dotnet/   Engine C# (.NET) — bản deploy chính:
│   ├── Program.cs              serve loop + thực thi action (brightness/CPU/refresh/wifi/GPU)
│   ├── State/                  SystemStateCollector — 15 state thật + app path
│   ├── Control/                HardwareControl — đặt brightness/CPU/refresh/wifi/GPU pref
...
│                    + hrl_integration.py (Manager tự gợi ý profile)
├── worldmodel/      world model (Phase 2) — dự phòng
├── advanced/        sac/ hierarchy/ planning/ memory(lstm) — kho nghiên cứu
├── app/             buy_business.py — GUI + license gate + chọn model theo profile
├── server/          server.py — FastAPI license server (tạo/gia hạn/thống kê key)
├── dashboard/       web Task-Manager-style + Chart.js offline + báo cáo tiết kiệm
├── engine/          (C++ cũ — không dùng trong bản deploy)
├── install.ps1      installer cho khách (shortcut + autostart + gỡ cài)
└── build_business.ps1
```
...
```
  ...
  │   App path (GPU)    ← MainModule.FileName (full path .exe)     │
  ├────────────────────────────────────────────────────────────────┤
  ...
  │   SetAppGpuPreference → registry per-app (iGPU/dGPU, hợp lệ)   │
...
```
   ...
   └───────────────────────────────────────────────────┘
        │  EvalCallback lưu best model (eval ở difficulty 3); LR giảm dần (ổn định)
        ▼
   export ONNX TỪ BEST MODEL → batteryclaw_policy.onnx (15→7)
```

3 MODEL PROFILE (cá nhân hóa thật) — train riêng bằng --profile:
```
   train.py --profile battery_saver → batteryclaw_policy_battery_saver.onnx
   train.py --profile balanced      → batteryclaw_policy.onnx (mặc định)
   train.py --profile performance   → batteryclaw_policy_performance.onnx
   App chọn model theo combobox Hồ sơ. 3 model có hành vi PHÂN HÓA rõ (lag thấp
   ↔ tiết kiệm gắt). Tự so điểm model mới vs cũ (.score) — giữ bản cũ nếu mới kém >5%.
```

Lựa chọn thuật toán khác (tùy chọn, chưa kiểm chứng tốt hơn PPO):
```
   train_sac.py  — SAC off-policy, xuất cùng ONNX 15→7 (thay PPO nếu muốn thử)
   advanced/ (SAC/HRL/MPC/LSTM) — kho nghiên cứu; LSTM deploy được dạng chuỗi
     (batch,30,15)→(batch,7) nhưng cần train riêng để giỏi.
   online/hrl_integration.py — Manager (HRL) tự gợi ý đổi profile theo pin/giờ/sạc.
```
...
```
   build_business.ps1 (Admin)
     ...
     └─ 4. → dist/BatteryClaw_business/  (zip gửi khách)
              ...
              ├── models/  (cả 3 onnx: balanced + battery_saver + performance)
              └── install.ps1  (tạo shortcut + tùy chọn autostart + gỡ cài)
```
